...
describe("DownloadModal", () => {
    ...
    test("renders modal when showModal = true", () => {
        render(<DownloadModal showModal={true} files={[]} closeModal={closeModalMock}/>);
        const modalHeader = screen.getByText("Downloaded Files");
        expect(modalHeader).toBeInTheDocument();
    });

    test("doesn't render modal when showModal = false", () => {
        render(<DownloadModal showModal={false} files={[]} closeModal={closeModalMock}/>);
    ...
    test("displays selected file information", () => {
        ...
        render(<DownloadModal showModal={true} files={[file1, file2]} closeModal={closeModalMock}/>);
